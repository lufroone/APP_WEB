...
async function connexion(){
    ...
    result = await fetch("http://hurflym.fr/arborescence_back+front/connexion_page_utilisateur.php", {method:"POST", body: formData}).then(res => res.json()).then(data => {return data;});
    fenetre_de_connexion.style.display = "none";
...
async function validation_information(section){
    if(section == '0'){
        ...
        await fetch("http://hurflym.fr/arborescence_back+front/changement_de_valeur_de_compte.php", {method:"POST", body: formData}).then(res => res.json()).then(data => {return data;});
        const container_a_modifier = document.getElementById("C_Nom_Utilisateur");
        var first = container_a_modifier.children[0];
        while (first) {
            first.remove();
...
async function deconnexion(){
    ...
    //await fetch("http://hurflym.fr/arborescence_back+front/destruction_de_session_et_de_tout_les_cookies.php", {method:"POST", body: formData}).then(res => res.json()).then(data => {return data;});
    window.location.replace("http://hurflym.fr/arborescence_back+front/index.php");
}
...
async function Changer_le_mot_de_passe_utilisateur(){
    ...
    if(password_2 == password_1){
        ...
        const res = await fetch("http://hurflym.fr/arborescence_back+front/changement_mot_de_passe_utilisateur.php", {method:"POST", body: formData}).then(res => res.json()).then(data => {return data;});
