...
async function connexion(){
    ...
    await recup_information_personalisation(info_Source[0]["ID_Source"],info_Source[0]["ID_Etablissement"]);
...
async function recup_information_personalisation(ID_Source,ID_Etablissement){
    console.log("ID_Source = " + ID_Source + "ID_Etablissement = "+ ID_Etablissement);
...
function Separer_les_informations_qui_ne_sont_pas_dans_le_signalement(){
    ...
    console.log(Information_equipement_present_dans_le_signalement);
...
async function envoi_du_mail_aux_administrateurs_concerne(ID_Noeud,ID_Etablissement,titre_du_mail,message_a_envoyer){
    alert(await fetch("http://hurflym.fr/arborescence_back+front/envoi_du_mail_a_ladministrateur.php?ID_Noeud="+ID_Noeud+"&ID_Etablissement="+ID_Etablissement+"&message="+message_a_envoyer+"&titre_du_mail="+titre_du_mail).then(res => res.json()).then(data => {return data;}));
}
