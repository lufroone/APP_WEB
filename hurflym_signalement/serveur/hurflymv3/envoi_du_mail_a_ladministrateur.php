<?php 
    ...
    header("Content-Type: text/javascript");
        ...
        $requete = $conn->prepare('SELECT * FROM Professionnel__Local WHERE Administrateur='.$_GET["ID_Etablissement"].' AND Lieu_de_travail='.$_GET["ID_Noeud"].'');
        $requete->execute();
        $info_administrateur = $requete->get_result();

        while($row = $info_administrateur->fetch_assoc()){
            if(isset($row["ID_Utilisateur"])){
                $requete_2 = $conn->query('SELECT * FROM Utilisateur__Local WHERE ID_Utilisateur='.$row["ID_Utilisateur"].'');
        ...
        echo '["'.$mail_de_ladministrateur["Mail_Utilisateur"].'"]';
